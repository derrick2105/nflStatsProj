...
class PopulateNFLDB:
    ...
    def populate_weekly(self, week, year):
        """
        This is a method that populates the database with a single week of
        stats. These stats include playerStats, injury_reports,
        and gameConditions.
        """

        self.populate_weather()
        self.populate_player_stats(week, year)
        self.populate_injury_report()
    ...
    def populate_players(self):
        ...
        if not self.DB.populate_db(statement1, Utilities.populate_log,
                                   insert_tuples1) \
            or not self.DB.populate_db(statement2, Utilities.populate_log,
                                       insert_tuples2):
    ...
    def populate_stats(self):
        ...
        try:
            ...
            if not self.DB.populate_db(statement, Utilities.populate_log,
                                       insert_tuples):
                Utilities.log("Problem executing statement in the database."
                              " Aborting.", Utilities.populate_log)
    ...
    def populate_new_season_games(self):
        ...
        max_season_id = self.DB.pull_from_db(get_statement1,
        ...
        games_max_id = self.DB.pull_from_db(get_statement2,
                                            Utilities.populate_log)
        ...
        if max_season_id != games_max_id:
            ...
            try:
                ...
                if not self.DB.populate_db(put_statement1,
                                           Utilities.populate_log,
                                           insert_tuples1):
                ...
                if not self.DB.populate_db(put_statement2,
                                           Utilities.populate_log,
                                           insert_tuples2):
                    Utilities.log("Problem executing statement in the "
                                  "database...Aborting.",
    ...
    def populate_games_from_data(self, data_path=None):
        """
    ...
    def populate_injury_report(self):
        ...
        """
        Utilities.log("Entering populate_injury_report.",
                      Utilities.populate_log)
        get_statement = 'select p.playerId from Players p, PlayerTeam t where' \
                        ' t.teamId <> "None" and t.playerId = p.playerId;'
        insert_statement = 'REPLACE into InjuryReport (playerId, ' \
                           'injurySeverity) values (%s, %s);'

        insert_tuples = []
        id_tuples = self.DB.pull_from_db(get_statement,
                                         Utilities.populate_log)
        ...
        for id_tup in id_tuples:
            ...
            try:
                ...
                Utilities.log_exception(e, Utilities.populate_log)
                Utilities.log("Exiting populate_injury_report.",
                              Utilities.populate_log)
                return False
        ...
        Utilities.log(message, Utilities.populate_log)
        if not self.DB.populate_db(insert_statement, Utilities.populate_log,
                                   insert_tuples):
            Utilities.log("Problem executing statement in the database. "
                          "Aborting.", Utilities.populate_log)
            Utilities.log("Exiting populate_injury_report.",
    ...
    def populate_weather(self):
        ...
        results = self.DB.pull_from_db(location_id_statement,
                                       Utilities.populate_log)
        if not results:
        ...
        results = self.DB.pull_from_db(game_statement, Utilities.populate_log,
        ...
        for key, value in weather_data['Games'].iteritems():
            ...
            if value['stadium'] == 'Sun Life Stadium':
                value['stadium'] = 'Hard Rock Stadium'
    ...
    def populate_locations(self):
        ...
        """
        Utilities.log("Entering populate_locations", Utilities.populate_log)
        get_statement = "select teamId, name from Teams;"
        get_statement2 = "select turf from TurfTypes;"
        ...
        try:
            ...
            teams_tuples = self.DB.pull_from_db(get_statement,
                                                Utilities.populate_log)

            turf_types = self.DB.pull_from_db(get_statement2,
                                              Utilities.populate_log)
            teams_dict = {}
            ...
            for stadium in stadiums:
                tup = (location_id, teams_dict[stadium[2]], stadium[0],
                       stadium[1])
                insert_tuples1.append(tup)
                if (stadium[1],) not in insert_tuples2 and \
                   (stadium[1],) not in turf_types:
            ...
            Utilities.log("Exiting populate_locations", Utilities.populate_log)
    ...
    def _get_season_id(self, week, year):
        statement = "select seasonId from Season where week = %s and " \
                    "seasonYear = %s;"
        res = self.DB.pull_from_db(statement, Utilities.populate_log,
                                   (week, year))
        if not res:
